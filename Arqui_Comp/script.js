  ...
  function createMovingBackground() {
    ...
    const drawParticle = (p) => {
      ...
      ctx.fill();
        ...
        const drawConnections = (items, maxDistance = 100) => {
      ctx.save();
      ctx.lineWidth = 0.5;
      ctx.globalAlpha = 0.15;
      for (let i = 0; i < items.length; i += 2) {
        for (let j = i + 1; j < items.length; j += 2) {
          const a = items[i];
          const b = items[j];
          const dx = a.x - b.x;
          const dy = a.y - b.y;
          ...
          ctx.strokeStyle = `rgba(170, 200, 255, ${1 - dist / maxDistance})`;
          ctx.beginPath();
          ctx.moveTo(a.x, a.y);
          ctx.lineTo(b.x, b.y);
          ctx.stroke();
...
const correct = SHUFFLED_QUESTIONS[current].ans;
    ...
    const reset = () => {
      ...
      addClass(DOM.quizContent, 'hidden');
      addClass(DOM.quizCounter, 'hidden');
      removeClass(DOM.quizStart, 'hidden');
      const panel = $(DOM.quizResult);
      if (panel) {
    ...
    const iniciarQuiz = () => {
      const startScreen = $(DOM.quizStart);
      if (startScreen) {
    ...
    return Object.freeze({ load, reset, iniciarQuiz });
  })();
  ...
  window.iniciarQuiz = () => safely(() => quiz.iniciarQuiz());
