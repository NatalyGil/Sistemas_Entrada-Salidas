...
(() => {
  ...
  function createMovingBackground() {
    ...
    particles = Array.from({ length: 100 }, () => createParticle(canvas.width, canvas.height));
...
const correct = SHUFFLED_QUESTIONS[current].ans;
  ...
  // WiFi vs Ethernet Simulator
  const wifiEthSim = (() => {
    let running = false;
    let time = 0;
    let irqEthernet = 0;
    let irqWifi = 0;
    let napiEthernet = false;
    let napiWifi = false;
    let animationId = null;

    const draw = () => {
      const canvas = document.getElementById('canvas-wifi');
      if (!canvas) return;
      const ctx = canvas.getContext('2d');
      const w = canvas.width;
      const h = canvas.height;

      // Si es la inicialización estática, no incrementar time
      if (!isStaticInit) {
        time++;
      ...
      ctx.fillText('¿Cómo llegan las interrupciones a la CPU?', w/2, 35);
      ...
      ctx.fillRect(ethX - 50, 90, 100, 55);
      ctx.strokeStyle = '#00ccff';
      ctx.lineWidth = 3;
      ctx.strokeRect(ethX - 50, 90, 100, 55);
      ctx.fillStyle = '#00ccff';
      ctx.font = '14px Arial';
      ctx.textAlign = 'center';
      ctx.fillText('Ethernet NIC', ethX, 125);

      // CPU
      ctx.fillStyle = '#334455';
      ctx.fillRect(ethX - 45, 290, 90, 85);
      ctx.fillStyle = '#ffffff';
      ctx.font = 'bold 14px Arial';
      ctx.fillText('CPU', ethX, 340);

      // Paquetes entrando (Ethernet)
      for (let i = 0; i < 5; i++) {
        let pos = (time * 2.5 + i * 50) % 250;
        if (pos > 40 && pos < 220) {
          ctx.fillStyle = '#00ffaa';
          ctx.font = 'bold 20px Arial';
          ctx.fillText('■', ethX - 20 + pos, 160);
        }
      }

      // Interrupciones Ethernet (menos frecuentes)
      if (time % 35 === 0) {
        irqEthernet++;
        napiEthernet = true;
      }
      if (time % 75 === 0) napiEthernet = false;
      ...
      if (napiEthernet) {
        ...
        ctx.setLineDash([6, 3]);
        ctx.beginPath();
        ctx.moveTo(ethX - 55, 260);
        ctx.lineTo(ethX + 55, 260);
        ctx.stroke();
        ctx.setLineDash([]);
        ctx.fillStyle = '#00ffff';
        ctx.font = 'bold 11px Arial';
        ctx.fillText('NAPI (Polling)', ethX, 278);
      }

      ctx.fillStyle = '#00ccff';
      ctx.font = 'bold 16px Arial';
      ctx.fillText('Ethernet', ethX, 70);
      ctx.font = 'bold 12px Arial';
      ctx.fillText('Interrupciones: ' + irqEthernet, ethX, 390);
      ...
      ctx.fillRect(wifiX - 50, 90, 100, 55);
      ctx.strokeStyle = '#ff44aa';
      ctx.lineWidth = 3;
      ctx.strokeRect(wifiX - 50, 90, 100, 55);
      ctx.fillStyle = '#ff44aa';
      ctx.font = '14px Arial';
      ctx.fillText('WiFi Chip', wifiX, 125);

      // Ondas WiFi
      let r = (time * 4) % 80;
      ctx.strokeStyle = `rgba(255, 70, 180, ${0.6 - r/150})`;
      ctx.lineWidth = 5;
      ctx.beginPath();
      ctx.arc(wifiX, 115, r + 25, 0, Math.PI * 2);
      ctx.stroke();

      // CPU
      ctx.fillStyle = '#334455';
      ctx.fillRect(wifiX - 45, 290, 90, 85);
      ctx.fillStyle = '#ffffff';
      ctx.font = 'bold 14px Arial';
      ctx.fillText('CPU', wifiX, 340);

      // Paquetes WiFi (más irregulares)
      for (let i = 0; i < 7; i++) {
        let pos = (time * 3.5 + i * 35) % 280;
        if (pos > 30 && pos < 240) {
          ctx.fillStyle = '#ff88cc';
          ctx.font = 'bold 18px Arial';
          ctx.fillText('◆', wifiX - 25 + pos * 0.9, 165);
        }
      }

      // Interrupciones WiFi (más frecuentes)
      if (time % 15 === 0) {
        irqWifi++;
        napiWifi = true;
      }
      if (time % 45 === 0) napiWifi = false;
      ...
      if (time % 15 < 7) {
        ...
        ctx.beginPath();
        ctx.setLineDash([2, 5]);
        ctx.moveTo(wifiX, 150);
        ctx.lineTo(wifiX, 285);
        ctx.stroke();
        ctx.setLineDash([]);
        ctx.fillStyle = '#ffff00';
        ctx.font = 'bold 16px Arial';
        ctx.fillText('⚡ IRQ', wifiX + 22, 220);
      }
      ...
      if (napiWifi) {
        ...
        ctx.setLineDash([5, 3]);
        ctx.beginPath();
        ctx.moveTo(wifiX - 55, 260);
        ctx.lineTo(wifiX + 55, 260);
        ctx.stroke();
        ctx.setLineDash([]);
        ctx.fillStyle = '#00ffff';
        ctx.font = 'bold 12px Arial';
        ctx.fillText('NAPI', wifiX, 278);
      }

      ctx.fillStyle = '#ff44aa';
      ctx.font = 'bold 16px Arial';
      ctx.fillText('WiFi', wifiX, 70);
      ctx.font = 'bold 12px Arial';
      ctx.fillText('Interrupciones: ' + irqWifi, wifiX, 390);
    ...
    let isStaticInit = false;

    // Dibuja una vez inicialmente (sin animación)
    const init = () => {
      const canvas = document.getElementById('canvas-wifi');
      if (!canvas) return;
      isStaticInit = true;
      draw(); // Dibuja una vez estática
      isStaticInit = false;
    };
    init();
    ...
    const start = () => {
      if (running) return;
      running = true;
      const btnIniciar = document.getElementById('btn-iniciar-wifi');
      const btnDetener = document.getElementById('btn-detener-wifi');
      if (btnIniciar) {
    ...
    return { start, stop, reset: () => { time = 0; irqEthernet = 0; irqWifi = 0; napiEthernet = false; napiWifi = false; } };
  })();
...
// Auto-resize en pestañas críticas
['dma-canvas', 'architecture-canvas', 'picos-canvas', 'timeline-canvas'].forEach(id => {
  window.addEventListener('resize', () => fixCanvasDPR(id));
});
